...
def ee_mas(funcion_objetivo, limites, iter_n, step_size, mu, lam, nombre_funcion):
    ...
    for epoch in range(iter_n):
        ...
        aptitudes = [funcion_objetivo(i, nombre_funcion) for i in poblacion]
        ...
        for i in range(n_hijos):
            ...
            while hijo is None or not in_bounds(hijo, limites):
                hijo = poblacion[i] + random.randn(len(limites)) * step_size
            hijos.append(hijo)
